...
async function main() {
  ...
  while (true) {
    ...
    console.log(`\n=================================================`);
    console.log(`👤 USER: ${currentUser.name || 'Unknown'} | ✉️  ${currentUser.email || 'No Email'} | 🆔 ${currentUser.user_id}`);
    console.log(`=================================================\n`);
    ...
    try {
      if (action.includes('Entitlements')) await manageEntitlements(currentUser.user_id);
      else if (action.includes('Quotas')) await manageQuotas(currentUser.user_id);
      else if (action.includes('Transactions')) await manageTransactions(currentUser.user_id);
...
async function manageEntitlements(userId: string) {
  ...
  if (!entitlements?.length) {
    ...
    console.table(entitlements.map(e => ({
      ...
      Status: e.status,
      'Valid Until': e.valid_until ? new Date(e.valid_until).toLocaleDateString() : 'Lifetime'
    })));
  }
  console.log('---------------------------------\n');
  ...
  if (action === 'Add/Update Feature') {
    ...
    const isLifetime = await new Confirm({ name: 'isLifetime', message: 'Is this a lifetime entitlement?' }).run();

    let validUntil = null;
    if (!isLifetime) {
      const days = await new NumberPrompt({ name: 'days', message: 'Valid for how many days?' }).run();
      const date = new Date();
    ...
    const { error } = await supabase.from('user_entitlements').upsert({
      user_id: userId, feature_id: featureId, status, valid_until: validUntil, updated_at: new Date().toISOString()
    });

    if (error) throw error;
    console.log(`✅ Feature ${featureId} updated successfully.`);
  } else if (action === 'Remove Feature') {
    const { error } = await supabase.from('user_entitlements').delete().eq('user_id', userId).eq('feature_id', featureId);
    if (error) throw error;
...
async function manageQuotas(userId: string) {
  ...
  if (!quotas?.length) {
    ...
    console.table(quotas.map(q => ({
      Feature: q.feature_id,
      Used: q.used,
      Limit: q.limit,
      'Reset At': q.reset_at ? new Date(q.reset_at).toLocaleDateString() : 'Never'
    })));
  }
  console.log('-------------------------\n');
  ...
  const { error } = await supabase.from('user_quotas').upsert({
    user_id: userId, feature_id: featureId, limit: newLimit, used: newUsed, updated_at: new Date().toISOString()
  });
...
async function manageTransactions(userId: string) {
  ...
  if (!transactions?.length) {
    ...
    console.table(transactions.slice(0, 10).map(t => ({
      ID: t.id.slice(0, 8),
      Type: t.type,
  ...
  if (action === 'Simulate Payment') {
    const type = await new Select({ name: 'type', message: 'Type:', choices: ['lifetime', 'subscription', 'top_up'] }).run() as TransactionType;
    const features: FeatureType[] = ['ai_scan', 'voice_reminder', 'lifetime_premium', 'ai_scanner_sub', 'voice_reminder_sub'];
    const featureId = await new Select({ name: 'feature', message: 'Feature:', choices: features }).run() as FeatureType;
    const amount = await new NumberPrompt({ name: 'amount', message: 'Amount in INR:' }).run();

    const { error } = await supabase.from('transactions').insert({
      user_id: userId, type, feature_id: featureId, amount: amount * 100, currency: 'INR', status: 'captured',
      razorpay_payment_id: `fake_${Date.now()}`, razorpay_order_id: `order_${Date.now()}`
    });
    ...
    const targetId = await new Select({
      name: 'id', message: 'Select transaction:',
      choices: transactions.map(t => ({ name: t.id, message: `${t.id.slice(0,8)} - ${t.type} - INR ${t.amount/100}` }))
    }).run();
    const newStatus = await new Select({
      name: 'status', message: 'New Status:',
    ...
    const targetId = await new Select({
      name: 'id', message: 'Select transaction to delete:',
      choices: transactions.map(t => ({ name: t.id, message: `${t.id.slice(0,8)} - ${t.type}` }))
    }).run();
