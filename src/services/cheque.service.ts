...
export class ChequeService {
  static async getAll(businessId: string): Promise<ChequeWithRelations[]> {
    ...
    return data as ChequeWithRelations[]
  }

  static async getById(id: string): Promise<ChequeWithRelations> {
    const { data, error } = await supabase
      .from('cheques')
      .select(
        '*, party:parties(name, color, icon, avatar_url), account:accounts(account_name, color, icon, bank:banks(name, logo_url))',
