...
export function useChequeStats(cheques: ChequeWithRelations[] | undefined) {
  return useMemo(() => {
    ...
    const upcomingCount = cheques.filter((c) => c.cheque_date > todayDate && c.status !== 'Cleared').length;
    const overdueCount = cheques.filter((c) => c.cheque_date < todayDate && c.status !== 'Cleared').length;
