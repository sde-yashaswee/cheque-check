    ...
    <div className="max-w-2xl space-y-12 pb-20">
      ...
      <section className="space-y-6">
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            <h2 className="text-xl font-bold tracking-tight">Lifetime Premium</h2>
            <p className="text-sm text-muted-foreground">One-time purchase for foundational features.</p>
          </div>
          {isLifetimePremium ? (
            ...
            <span className="text-sm font-semibold text-primary">₹300 One-time</span>
          )}
        </div>
        ...
        {!isLifetimePremium && (
          <div className="bg-primary/5 border border-primary/10 rounded-xl p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground">Get lifetime access to all base features.</p>
            </div>
            <Button 
      ...
      <section className="space-y-6">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">Monthly subscriptions for high-value services.</p>
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between">
                  ...
                  {isSubscribed ? (
                    <Badge className="bg-primary/10 text-primary border-none">Active Subscription</Badge>
                  ) : isLocked ? (
                    <Badge variant="secondary" className="bg-yellow-100 text-yellow-800 border-none">Locked</Badge>
                  ) : (
                    <Badge variant="secondary" className="bg-blue-100 text-blue-800 border-none">Trial Access</Badge>
                  )}
                </div>
                ...
                <div className="space-y-4">
                  <div className="space-y-1">
                    <div className="flex items-center justify-between">
                      ...
                      {!isSubscribed && <span className="text-sm font-medium text-primary">{addon.price}</span>}
                    </div>
                    <p className="text-sm text-muted-foreground leading-relaxed">
                      {addon.description}
                    </p>
                  ...
                  {(isSubscribed || hasQuota) && addon.quota && (
                    <div className="space-y-2 rounded-lg bg-primary/5 p-4">
                      <div className="flex justify-between text-xs font-bold uppercase tracking-wider">
                        ...
                        <span className="text-primary">{addon.quota.used} / {addon.quota.limit}</span>
                      </div>
                      <Progress value={(addon.quota.used / addon.quota.limit) * 100} className="h-2" />
                      {isSubscribed && (
                        <div className="flex justify-end pt-1">
                          ...
                          >
                            <HugeiconsIcon icon={ZapIcon} className="w-3 h-3 mr-1" />
                            Buy Top-up
                          </button>
                        </div>
                      )}
                  ...
                  {!isSubscribed && (
                    ...
                    >
                      ...
                      {isLocked ? `Subscribe for ${addon.price}` : "Upgrade Subscription"}
                    </Button>
                  )}
                </div>
              </div>
