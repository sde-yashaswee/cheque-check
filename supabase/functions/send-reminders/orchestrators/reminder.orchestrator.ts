...
import { ReminderResult } from '../types.ts';
...
export class ReminderOrchestrator {
  ...
  async run(): Promise<ReminderResult[]> {
    ...
    const userIds = [...new Set(cheques.map((c: any) => c.business.user_id))];
    ...
    for (const profile of profiles) {
      const userCheques = cheques.filter((c: any) => c.business.user_id === profile.user_id);
      
      // Determine user language, fallback to english
      const lang = profile.language || 'en';
      const message = this.buildMessage(profile.name || 'User', userCheques, lang);
      
      // Pass the message and language down to the callback URL
      const twimlUrl = new URL(this.baseUrl);
      twimlUrl.searchParams.set('action', 'twiml');
      twimlUrl.searchParams.set('message', message);
      twimlUrl.searchParams.set('lang', lang);

      Logger.info(`Initiating call to ${profile.phone} for ${profile.name} in ${lang}`);
      const callResult = await this.twilio.initiateCall(profile.phone!, twimlUrl.toString());
  ...
  private buildMessage(name: string, cheques: any[], language: string): string {
    const banks: Record<string, { count: number; total: number }> = {};
    cheques.forEach(c => {
      const bankName = c.account?.bank?.name || 'Unknown Bank';
      if (!banks[bankName]) banks[bankName] = { count: 0, total: 0 };
      banks[bankName].count++;
      banks[bankName].total += Number(c.amount);
    });

    if (language === 'hi') {
      const bankSummaries = Object.entries(banks)
        .map(([bank, data]) => `${bank} में ${data.total} रुपये के ${data.count} चेक`)
        .join(', और ');
      ...
      const bankSummaries = Object.entries(banks)
        .map(([bank, data]) => `${data.count} cheque${data.count > 1 ? 's' : ''} hitting ${bank} for a total of ${data.total}`)
        .join(', and ');
